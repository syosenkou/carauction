 ...
 * @param {org.example.biznet.Trade} tx - the trade to be processed
 * @transaction
 */
async function tradeCommodity(tx) { 
    ...
    const farmerRegistry = await getParticipantRegistry('org.example.biznet.Farmer');
    await farmerRegistry.update(contract.farmer);


    // update the grower's balance
    const traderRegistry = await getParticipantRegistry('org.example.biznet.Trader');
 ...
 * @param {org.example.biznet.SetupDemo} setupDemo - the SetupDemo transaction
 * @transaction
 */
